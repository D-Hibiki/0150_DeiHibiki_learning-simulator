...
- 本リポジトリの独自コード、文書、画像およびプレゼン資料は、別途明記がない限り [LICENSE](LICENSE) の対象です。リポジトリをPublicで閲覧可能にすることは、利用・複製・改変・再配布の許諾を意味しません。
- 第三者ライブラリには各配布元のライセンスが適用されます。直接依存と固定versionは [`package.json`](package.json) と [`package-lock.json`](package-lock.json) を正本とし、本リポジトリのLICENSEが第三者ライブラリのライセンスを変更することはありません。
- シミュレーション入力と結果は合成データです。実在する個人、学校、組織の調査データや外部データセットを収録していません。
- Model v1の数式、Agent Worldの設計、実験条件、制約および再現手順は、本リポジトリ内の [方法論](docs/METHODOLOGY.md)、[ODD仕様](docs/agent-world-odd.md)、[実行結果](RESULTS.md) に記録しています。
- 公開前後の検査範囲とGitHub側の残作業は [PUBLIC_READY.md](PUBLIC_READY.md) に記録します。
